 ...
 * Author: ashwi
 *
 * Created on 8 October, 2024, 9:51 PM
 */
...
void putch(unsigned char byte)    //fcn to transmit 1 byte 
{
	/* Output one byte */
	/* Set when register is empty */
	while(!TXIF)  // run the while loop until TXREG register is empyt so tht we can load new data
	{
		continue;
	}
	TXIF = 0; 
	TXREG = byte;  // after TXREG is empty assign with our data
} 

int puts(const char *s)   //fcn to  transmit a string
{
	while(*s)		
	{
		putch(*s++);	
...
unsigned char getch(void)    // receive 1 byte
{
	...
	while(!RCIF)   // wait until receive reg is full
	{
		continue;
	}
	RCIF = 0;
...
unsigned char getche(void)   // receive & then transmit 1 byte
{
	unsigned char c;
